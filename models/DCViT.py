...
# DC LDC starts at first patch embedding ctrl + f (start here)
...
class Block(nn.Module):
    def __init__(
        ...
        # decide how many more DC LDC to add start from second block
        ...
        if 0 < self.th_layer <= self.LDC_stop_layer:
            ...
            self.learnable_mask_hv = nn.Parameter(
                torch.ones([self.conv_hv.weight.size(0), self.conv_hv.weight.size(1)]),
                requires_grad=True)
            ...
            self.learnable_mask_dc = nn.Parameter(
                torch.ones([self.conv_dc.weight.size(0), self.conv_dc.weight.size(1)]),
                requires_grad=True)
            self.theta_dc = nn.Parameter(torch.ones(1) * 0.3, requires_grad=True)
            
            # Change the ratio of DC LDC (lower by 0.10 each block)
            self.ratio = 0.25 - self.LDC_stop_layer*0.10
    ...
    def forward(self, x):
        if 0 < self.th_layer <= self.LDC_stop_layer:
            ...
            x_hv = F.conv2d(input=x_re, weight=self.conv_hv.weight * mask, bias=self.conv_hv.bias,
                            stride=self.conv_hv.stride,
            ...
            # data = data + x_hv * 0.15 + x_dc * 0.15
            data = data + x_hv * self.ratio + x_dc * self.ratio
...
class VisionTransformer(nn.Module):
    def __init__(
        ...
        self.blocks = nn.Sequential(*[
            block_fn(
                ...
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, act_layer=act_layer,th_layer=i)
    ...
    def forward_features(self, x):
        ...
        x_dc = F.conv2d(input=x_re, weight=self.conv_dc.weight * mask, bias=self.conv_dc.bias,
                        ...
                        padding=self.conv_dc.padding, groups=self.conv_dc.groups)
        x_dc = x_dc.permute(0, 3, 1, 2).reshape(batch_size, 196, 768)
        x_dc = self._pos_embed(x_dc)

        x = self._pos_embed(x)
        
        x = x + x_hv * 0.25 + x_dc * 0.25
